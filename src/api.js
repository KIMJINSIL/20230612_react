...
export async function comicsList(){
    return await fetch(`${BASE_PATH}/v1/public/comics?apikey=${API_KEY}`).then(res =>
    res.json()
  );
}

export async function eventsList(){
    return await fetch(`${BASE_PATH}/v1/public/events?orderBy=name&apikey=${API_KEY}`).then(res =>
      res.json()
    );
}
...
export async function charactersList(props){
    ...
    console.log(props);
    return await fetch(`${BASE_PATH}/v1/public/characters?limit=${customLimit}&offset=${offset}&apikey=${API_KEY}`).then(res =>
    res.json()
  )
}
